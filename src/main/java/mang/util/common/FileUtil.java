 ...
 * @version create:2016-10-10 10:24:29 modfiy:2016-10-10 10:24:33
 * 
 */
public class FileUtil {
	...
	/**
	 * 删除某目录下的所有文件. 删除某一路径下所有文件 但是该目录不删除 即清空该目录
	 * 
	 * @param foldPath
	 *            foldPath
	 * @return boolean
	 ...
	 * 文件、文件夹都能删除
	 * <ul>
	 * <li>目录路径示例 c:/Users/mang/Desktop/haiguan/Send/test/ 或者
	 * c:/Users/mang/Desktop/haiguan/Send/test 都行
	 ...
	 * 
	 * @param sourcePath
	 *            源文件路径 可以是文件也可以是目录 如c:/Users/mang/Desktop/haiguan/Send/
	 * @param destPath
	 *            目标目录 必须是一个目录
	 * @param isOverWrite
	 *            是否覆盖 如果移动时有同名是否覆盖
	 ...
	 * 将一个目录下的文件移动到另一个目录下.
	 * <p>
	 * 是将该目录下的文件移到到另一个目录 不是该目录<br>
	 * 如a 目录下有 a1,a2,移动到b目录 则b目录下有a1,a2
	 * 
	 * @param sourceFoldPath
	 *            源目录
	 * @param destFoldPath
	 *            目标目录
	 * @param isCleanDestFold
	 *            是否清空目录目录 即如果目录下已经有文件是否清空该目录
	 *            <ul>
	 *            <li>如果目标目录下已经有文件 可根据isCleanDestFold 决定是否清空目录目录</li>
	 *            <li>如果目标目录下有文件
	 *            也不配置了不清空目标目录,则仍然可以将源目录中不同名文件移动到目标路径下（如果同名按照isOverWrite的配置进行处理）(如海关出入卡口xml本地生成路径与移除路径一致时使用过)</li>
	 *            </ul>
	 * @param isOverWrite
	 *            是否覆盖 如果移动时有同名文件是否覆盖 true表示覆盖 false表示不覆盖
	 *            <ul>
	 *            <li>如果移动时出现同名文件 则根据isOverWrite进行处理处理</li>
	 *            <li>如果移动时出现了同名文件 则肯定是目标目录存在并且配置了isCleanDestFold为false</li>
	 *            </ul>
	 */
	public static void moveFoldfilesToAnotherFold(String sourceFoldPath, String destFoldPath, boolean isCleanDestFold,
			boolean isOverWrite) {
	 ...
	 * 则处理成c:\Users\mang\Desktop\haiguan\Send\ <br>
	 * 如果是在linux下，则如果传来 /home/oracle 会处理成 /home/oracle/
	 * 注：有时在windows下你也可能把路径写成c:/Users/mang/Desktop/haiguan/Send
	 * 然后处理成c:/Users/mang/Desktop/haiguan/Send\ 但经测试不影响使用
	 * 
	 * @param path
	 *            path
	 * @return String
	...
	/**
	 * 根据文件名获取文件后缀名. 如文件名是 test1.xlsx 则返回xlsx
	 * 
	 * @param fileName
	 *            文件名
	 * @return 返回文件后缀名
